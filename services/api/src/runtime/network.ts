...
export async function ensureSharedContainerConnected(
  ...
  const isConnected = await sandbox.provider.isConnectedToNetwork(containerName, networkName);
  if (isConnected) {
    return true;
  }
...
export async function ensureSharedContainerConnectedToActiveSessions(
  ...
  for (const { id: sessionId } of activeSessions) {
    const didConnect = await ensureSharedContainerConnected(sessionId, containerName, sandbox);
    if (didConnect) {
      connected++;
    } else {
      missingNetworks++;
